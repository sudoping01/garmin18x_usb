...
class GpsCorrectionTest:
	...
	def __init__(self):
		...
		self.rover_odom = BaseToRover()

		while not rospy.is_shutdown():
			pass

	def update_data(self, fix_in):
		if self.fix.status.status == 1:
			rlat2 = fix_in.latitude * pi / 180
			rlon2 = fix_in.longitude * pi / 180
			dlat =  rlat2 - self.rlat1
			dlon =  rlon2 - self.rlon2
			a = sin(dlat / 2)**2 + cos(lat1) * cos(lat2) * sin(dlon/2)**2
			c = 2 * asin(sqrt(a))
			self.rover.distance = c * 6371
			self.rover.bearing = atan2( sin(lon2 - self.lon1) * cos(lat2), cos(self.lat1) * sin(lat2) - sin(self.lat1) * cos(lat2) * cos(lon2 - self.lon1))
			self.rover.rel_alt = fix_in.alt - self.base_alt
